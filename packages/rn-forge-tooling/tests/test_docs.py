...
class TestCheckStructure:
    ...
    def test_superseded_status_is_allowed(self, repo):
        ...
        assert "docs.adr-status" not in codes(check_structure(repo, repo / "docs"))

    def test_broken_link_is_reported(self, repo):
        write(repo / "docs/guides/setup.md", "# Setup\n\n[gone](missing.md)\n")
        assert "docs.broken-link" in codes(check_structure(repo, repo / "docs"))

    def test_broken_anchor_is_reported(self, repo):
        write(repo / "docs/guides/setup.md", "# Setup\n\n[x](index.md#nope)\n")
        assert "docs.broken-anchor" in codes(check_structure(repo, repo / "docs"))
    ...
    def test_release_and_epic_naming(self, repo):
        ...
        found = codes(check_structure(repo, repo / "docs"))
        assert "docs.release-naming" in found
        assert "docs.feature-naming" in found

    def test_missing_areas_manifest_is_one_finding(self, repo):
        (repo / "docs/_areas.yml").unlink()
        assert codes(check_structure(repo, repo / "docs")) == ["docs.areas-manifest"]
    ...
    def test_no_instruction_file_is_reported(self, repo):
        ...
        assert codes(check_structure(repo, repo / "docs")) == [
...
class TestDocsCli:
    ...
    @pytest.fixture
    def cli(self):
        from rn_forge.tooling.docs.cli import app

        return app

    def test_structure_passes_on_a_valid_tree(self, runner, cli, repo):
        result = runner.invoke(cli, ["structure", "--root", str(repo)])
    ...
    def test_json_output_carries_the_findings(self, runner, cli, repo):
        ...
        (repo / "docs/guides/_structure.md").unlink()
        result = runner.invoke(cli, ["--json", "structure", "--root", str(repo)])
        assert result.exit_code == 1
