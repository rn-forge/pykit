...
from rn_forge.commons.lang.types import VerboseLoggerBase
...
class LoggingConfig:
    ...
    def defaults(cls) -> dict[str, Any]:
        ...
        return {
            ...
            "isatty": isatty,  # auto-detected from sys.stderr at configuration time
            "enable_color": isatty,  # colour on when the log stream is a TTY
    ...
    def _configure_logging(config: LoggingConfig, file_path: str | None) -> None:
        ...
        if use_rich:
            ...
            console_handler = {
                ...
                "formatter": console_formatter_name,
                "filters": ["enrich"],
                # Logs are diagnostics, not results. They go to stderr so that
                # a command's stdout carries only its structured output — a
                # `--json` payload stays parseable even when the flag was given
                # after the subcommand name, too late for the root callback to
                # have suppressed a default-level log line.
...
def _build_rich_console(config: LoggingConfig) -> Console:
    ...
    if not config.level_styles:
        return Console(stderr=True, theme=_RICH_THEME)
    overrides = {
        f"logging.level.{name.lower()}": style
        for name, style in config.level_styles.items()
    }
    return Console(stderr=True, theme=Theme({**_RICH_LEVEL_STYLES, **overrides}))
